...
const Projects = () => {
  ...
  return (
    <section id="projects" className="bg-[#F4F4F0] flex flex-col divide-y divide-[#111111]">

      {/* Header */}
      <div className="p-8 md:p-12 bg-[#111111] text-[#F4F4F0] flex justify-between items-end border-b border-[#111111]">
        <div>
          <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-gray-400 mb-4">/ SELECTED WORKS</h2>
          <h3 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none">Featured<br />Projects<span className="text-[#FF3333]">.</span></h3>
        </div>
        <div className="hidden md:block text-sm font-bold uppercase tracking-widest text-gray-400 text-right">
          {projects.length < 10 ? `0${projects.length}` : projects.length} Items <br /> 2023 - 2024
        </div>
      </div>

      {/* Projects List */}
      <div className="flex flex-col bg-[#E5E5E5] p-4 md:p-8 gap-8">
        {currentProjects.map((project, index) => {
          const globalIndex = (currentPage - 1) * itemsPerPage + index;
          return (
            <div key={globalIndex} className="bg-[#F4F4F0] border border-[#111111] grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-[#111111] group hover:-translate-x-1 hover:translate-y-1 hover:shadow-[8px_8px_0_0_#111111] transition-transform duration-100 ease-linear cursor-pointer">

              {/* Info Block */}
              <div className={`p-8 md:p-12 flex flex-col justify-between ${project.layout === 'right' ? 'lg:order-last' : ''}`}>
                <div className="flex justify-between items-start mb-12">
                  <span className="text-xs font-bold uppercase tracking-widest text-[#F4F4F0] bg-[#111111] border border-[#111111] px-3 py-1 group-hover:bg-[#FF3333] group-hover:border-[#FF3333] transition-colors duration-100">
                    {project.category}
                  </span>
                  <span className="text-xl font-black">{globalIndex + 1 < 10 ? `0${globalIndex + 1}` : globalIndex + 1}</span>
                </div>

                <div>
                  <h4 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-6 group-hover:text-[#FF3333] transition-colors duration-100">
                    {project.title}
                  </h4>
                ...
                <div className="mt-12 flex flex-wrap gap-6">
                  {project.demoLink && (
                    <a href={project.demoLink} target="_blank" rel="noreferrer" className="text-sm font-bold uppercase tracking-widest border-b-2 border-[#111111] pb-1 hover:text-[#FF3333] hover:border-[#FF3333] transition-colors duration-100">
                      Live Demo -&gt;
                    </a>
                  )}
                  {project.repoLink && (
                    <a href={project.repoLink} target="_blank" rel="noreferrer" className="text-sm font-bold uppercase tracking-widest border-b-2 border-[#111111] pb-1 hover:text-[#FF3333] hover:border-[#FF3333] transition-colors duration-100">
                      Source Code -&gt;
                    </a>
                  )}
                </div>
              ...
              <div className={`w-full h-64 lg:h-auto border-[#111111] overflow-hidden ${project.layout === 'right' ? 'border-b lg:border-b-0' : 'border-t lg:border-t-0'}`}>
                <a
                  href={project.demoLink && project.demoLink !== "#" ? project.demoLink : (project.repoLink && project.repoLink !== "#" ? project.repoLink : "#")}
                  target={project.demoLink !== "#" || project.repoLink !== "#" ? "_blank" : "_self"}
                  rel="noreferrer"
                  className="block w-full h-full"
                >
                  <img
                    src={project.image || `https://images.unsplash.com/photo-${1550000000000 + globalIndex}?auto=format&fit=crop&q=80&w=1000`}
                    alt={project.title}
                    ...
                    onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1618401471353-b98afee0b2eb?auto=format&fit=crop&q=80&w=1200"; }}
                  />
                </a>
              </div>
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex justify-between items-center p-8 md:p-12 bg-[#F4F4F0] border-t border-[#111111]">
          <button
            onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
            disabled={currentPage === 1}
            className="text-xs md:text-sm font-bold uppercase tracking-widest px-6 py-3 border border-[#111111] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#111111] hover:text-[#F4F4F0] transition-colors cursor-pointer bg-[#F4F4F0] text-[#111111]"
          >
            &lt;- Prev Page
          </button>
          <span className="text-xs md:text-sm font-bold uppercase tracking-widest text-[#111111]">
